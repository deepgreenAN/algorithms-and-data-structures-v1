...
/// Todoリストを表す列挙体
struct TodoList<'a, T> {
    stacklike: Box<dyn StackLike<Item = T> + 'a>,
}

impl<'a, T> TodoList<'a, T> {
    fn new(new_vec: Vec<T>, search_method: SearchMethod) -> Self
    where
        T: 'a,
    {
        match search_method {
            SearchMethod::Depth => Self {
                stacklike: Box::new(Stack::new(new_vec)),
            },
            SearchMethod::Breadth => Self {
                stacklike: Box::new(Queue::new(new_vec)),
            },
        }
    }
    fn push(&mut self, x: T) {
        self.stacklike.push(x)
    }
    fn pop(&mut self) -> Option<T> {
        self.stacklike.pop()
    }
    fn is_empty(&self) -> bool {
        self.stacklike.is_empty()
    }
}
